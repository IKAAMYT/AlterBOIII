...
std::wstring get_webview_data_path() {
  ...
  auto path = std::filesystem::path(local_app_data) / "AlterBO3" / "WebView2";
...
void html_frame::show_webview_error(const HRESULT result) const {
  ...
  MessageBoxW(this->window_, message.c_str(), L"AlterBO3",
              MB_OK | MB_ICONERROR);
...
void html_frame::initialize_legacy_browser() {
  ...
  if (FAILED(result) || !this->browser_object_) {
    MessageBoxW(this->window_,
                ...
                L"AlterBO3", MB_OK | MB_ICONERROR);
    PostMessageW(this->window_, WM_CLOSE, 0, 0);
    return;
  }

  this->browser_object_->SetClientSite(this);
  this->browser_object_->SetHostNames(L"AlterBO3", nullptr);
  RECT bounds{};
  GetClientRect(this->window_, &bounds);
