...
local function addFriendsAccessOption(options, controller)
  ...
  table.insert(options, {
    models = {
      displayText = friendsCanJoin and "AUTORISER LES AMIS" or "OUVRIR AUX AMIS",
...
local customStartMenuGameOptions = ListHelper_SetupDataSource("StartMenuGameOptions", function(controller)
  ...
  elseif isMP then
    ...
    local endGameText = isLocalHost and "MENU PRINCIPAL" or "MENU_QUIT_GAME_CAPS"
    endGameOption = { models = { displayText = endGameText, action = QuitGame_MP } }
    if isLocalHost then
      restartGameOption = {
        models = {
          displayText = "REDÉMARRER LA PARTIE",
          action = restartGame,
        },
      }
    end
  ...
  if isLocalHost then
    ...
    table.insert(options, {
      models = {
        displayText = "EXPULSER UN JOUEUR",
        action = function(self, element, controller, param, menu)
          if menu and menu.openPopup then
  ...
  table.insert(options, { models = { displayText = "QUITTER VERS LE BUREAU", action = OpenPCQuit } })
  return options
end, true)
